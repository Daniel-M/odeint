...
func NewRk4(stepSize Float, system System) (r *Rk4) {
	if stepSize <= 0 {
...
func (rk4 *Rk4) SetStep(step Float) error {
	rk4.stepSize = step
	return nil
}

// SetState sets the state for the Rk4 stepper.
func (rk4 *Rk4) SetState(state []Float) error {
	rk4.system.stateVector = state
	return nil
}

// Set sets the step size and System for the Rk4 stepper.
func (rk4 *Rk4) Set(stepSize Float, system System) error {
	if stepSize <= 0 {
...
func (rk4 *Rk4) StepSize() Float {
	return rk4.stepSize
}

// State returns the state of the Rk4 stepper.
func (rk4 *Rk4) State() []Float {
...
func (rk4 *Rk4) Step() ([]Float, error) {

	newstate := rk4.system.stateVector
	stateK1 := make([]Float, len(rk4.system.stateVector))
	stateK2 := make([]Float, len(rk4.system.stateVector))
	stateK3 := make([]Float, len(rk4.system.stateVector))
	stateK4 := make([]Float, len(rk4.system.stateVector))

	BufferK1 := make([]Float, len(rk4.system.stateVector))
	BufferK2 := make([]Float, len(rk4.system.stateVector))
	BufferK3 := make([]Float, len(rk4.system.stateVector))
