...
func NewEuler(stepSize float32, system System) (r *Euler) {
	if stepSize <= 0 {
...
func (euler *Euler) SetStep(step float32) error {
	euler.stepSize = step
	return nil
}

// SetState sets the state for the Euler stepper.
func (euler *Euler) SetState(state []float32) error {
	euler.system.stateVector = state
	return nil
}

// Set sets the step size and System for the Euler stepper.
func (euler *Euler) Set(stepSize float32, system System) error {
	if stepSize <= 0 {
...
func (euler *Euler) Step() ([]float32, error) {

	newstate := make([]float32, len(euler.system.stateVector))
