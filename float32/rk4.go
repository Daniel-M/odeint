...
func NewRk4(stepSize float32, system System) (r *Rk4) {
	if stepSize <= 0 {
...
func (rk4 *Rk4) SetStep(step float32) error {
	rk4.stepSize = step
	return nil
}

// SetState sets the state for the Rk4 stepper.
func (rk4 *Rk4) SetState(state []float32) error {
	rk4.system.stateVector = state
	return nil
}

// Set sets the step size and System for the Rk4 stepper.
func (rk4 *Rk4) Set(stepSize float32, system System) error {
	if stepSize <= 0 {
...
func (rk4 *Rk4) StepSize() float32 {
	return rk4.stepSize
}

// State returns the state of the Rk4 stepper.
func (rk4 *Rk4) State() []float32 {
...
func (rk4 *Rk4) Step() ([]float32, error) {
	...
	stateK1 := make([]float32, len(rk4.system.stateVector))
	stateK2 := make([]float32, len(rk4.system.stateVector))
	stateK3 := make([]float32, len(rk4.system.stateVector))
	stateK4 := make([]float32, len(rk4.system.stateVector))

	BufferK1 := make([]float32, len(rk4.system.stateVector))
	BufferK2 := make([]float32, len(rk4.system.stateVector))
	BufferK3 := make([]float32, len(rk4.system.stateVector))
